interface CategoryScore {
    name: string;
    score: number;
    type: 'Hard Skills' | 'Soft Skills' | 'Experiencia' | 'Educación';
}

interface MissingHardSkill {
    name: string;
    icon: string;
}

interface MissingSoftSkill {
    name: string;
    status: 'Parcial' | 'Requerido';
    statusColor: string;
    description: string;
}
...
interface GapAnalysisData {
    ...
    missingHardSkills: MissingHardSkill[];
    missingSoftSkills: MissingSoftSkill[];
    experienceGaps: ExperienceGap[];
    aiRecommendation: string;
}

const mockAnalysisData: GapAnalysisData = {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (trendBadge) trendBadge.textContent = mockAnalysisData.matchTrend;

    const rateText = document.getElementById("match-rate-text");
    if (rateText) rateText.textContent = `${mockAnalysisData.matchRate}%`;

    const rateBar = document.getElementById("match-rate-bar");
    if (rateBar) rateBar.style.width = `${mockAnalysisData.matchRate}%`;

    const rateSummary = document.getElementById("match-rate-summary");
    if (rateSummary) rateSummary.textContent = mockAnalysisData.matchSummary;
    ...
    if (aiRecText) aiRecText.textContent = mockAnalysisData.aiRecommendation;
});
