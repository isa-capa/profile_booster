export interface Application {
    ...
    role: string;
    date: string;
    status: 'Entrevista' | 'Aplicado' | 'Rechazado' | 'Oferta';
    statusClasses: string;
    dotColorClass: string;
}

export interface DashboardStats {
    sent: { value: number; trend: string; trendIcon: string };
    interviews: { value: number; trend: string; trendIcon: string };
    offers: { value: string; trend: string; trendIcon: string };
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sentEl) sentEl.textContent = stats.sent.value.toString();
    ...
    if (intEl) intEl.textContent = stats.interviews.value.toString();
    ...
    if (offEl) offEl.textContent = stats.offers.value.toString();
