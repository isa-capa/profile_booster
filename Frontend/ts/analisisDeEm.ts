...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (nameBreadcrumb) nameBreadcrumb.textContent = data.companyName;

    const nameTitle = document.getElementById("company-name-title");
    if (nameTitle) nameTitle.textContent = data.companyName;

    // Inject executive summary
    const bmText = document.getElementById("business-model-text");
    if (bmText) bmText.textContent = data.businessModel;

    const sectorText = document.getElementById("sector-context-text");
    if (sectorText) sectorText.textContent = data.sectorContext;
    ...
    if (missionText) missionText.textContent = `"${data.corporateMission}"`;
    ...
    if (personaText) personaText.textContent = data.targetPersona;
    ...
    if (errorsText) errorsText.textContent = data.commonErrors;

    const insightText = document.getElementById("insight-pro-text");
    if (insightText) insightText.textContent = data.insightPro;
});
