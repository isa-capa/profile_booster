...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (cvName) cvName.textContent = mockCV.name;
    if (cvRole) cvRole.textContent = mockCV.role;
    if (cvContact) cvContact.textContent = mockCV.contact;
    if (cvLinks) cvLinks.textContent = mockCV.links;
    ...
    if (cvProfile) cvProfile.textContent = mockCV.profile;
    if (cvTechSkills) cvTechSkills.textContent = mockCV.techSkills;
    if (cvSoftSkills) cvSoftSkills.textContent = mockCV.softSkills;
    ...
    if (properMatchScoreText) properMatchScoreText.textContent = `${mockSidebar.matchScore}%`;
    ...
    if (detectedCountText) detectedCountText.textContent = `Detectadas (${mockSidebar.detectedKeywords.length})`;
    if (detectedContainer) {
        detectedContainer.innerHTML = mockSidebar.detectedKeywords.map(kw =>
            `<span class="px-2.5 py-1 bg-green-500/10 text-green-700 dark:text-green-300 text-xs font-medium rounded border border-green-500/20">${kw}</span>`
        ).join("");
    }
    ...
    if (missingCountText) missingCountText.textContent = `Faltantes (${mockSidebar.missingKeywords.length})`;
    if (missingContainer) {
        missingContainer.innerHTML = mockSidebar.missingKeywords.map(kw =>
            `<span class="px-2.5 py-1 bg-orange-500/10 text-orange-700 dark:text-orange-300 text-xs font-medium rounded border border-orange-500/20 dashed border-dashed">${kw}</span>`
        ).join("");
    }
});
