...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (subtitleElement) subtitleElement.textContent = mockSummaryData.pageSubtitle;

    const heroBadge = document.getElementById("hero-badge");
    if (heroBadge) heroBadge.innerHTML = `<span class="material-symbols-outlined text-[14px] fill-1">verified</span> ${mockSummaryData.heroBadge}`;

    const heroTitle = document.getElementById("hero-title");
    if (heroTitle) heroTitle.textContent = mockSummaryData.heroTitle;

    const heroDescription = document.getElementById("hero-description");
    if (heroDescription) heroDescription.innerHTML = mockSummaryData.heroDescription;
